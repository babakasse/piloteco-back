...
final class Version20250906150000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql("INSERT INTO user (email, first_name, last_name, roles, password, company_id) VALUES 
            ('demo.admin@piloteco.fr', 'Admin', 'Demo', '[\"ROLE_ADMIN\"]', '$hashedPassword', (SELECT id FROM company WHERE name = 'EcoTech Solutions' LIMIT 1)),
            ('demo.manager@ecotech.fr', 'Marie', 'Dubois', '[\"ROLE_USER\"]', '$hashedPassword', (SELECT id FROM company WHERE name = 'EcoTech Solutions' LIMIT 1)),
            ('demo.analyst@greenmanuf.fr', 'Pierre', 'Martin', '[\"ROLE_USER\"]', '$hashedPassword', (SELECT id FROM company WHERE name = 'Green Manufacturing Co.' LIMIT 1)),
            ('demo.consultant@sustainable.fr', 'Sophie', 'Durand', '[\"ROLE_USER\"]', '$hashedPassword', (SELECT id FROM company WHERE name = 'Sustainable Logistics' LIMIT 1)),
            ('demo.expert@cleanenergy.fr', 'Thomas', 'Leroy', '[\"ROLE_USER\"]', '$hashedPassword', (SELECT id FROM company WHERE name = 'CleanEnergy Corp' LIMIT 1))
        ");
        ...
        $this->addSql("INSERT INTO emission (source, category, amount, scope, description, carbon_assessment_id) VALUES 
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql("DELETE FROM emission WHERE carbon_assessment_id IN (
